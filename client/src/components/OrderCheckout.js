...
const OrderCheckout = ({ order, setOrder, products }) => {
  ...
  useEffect(() => {
    setTotal(
      order.reduce(function (sum, product) {
  ...
  const handleOrderClick = () => {
    ...
    const message = `Su pedido de ${totalGlobal.toFixed(
      2
    )}€ llegará en unos ${Math.floor(
  ...
  return (
    <div className="h-full bg-[rgb(245,245,245)]">
      <div
        className={`flex justify-around pt-16 pb-8 items-center uppercase ${
          isXs || isSm || isMd ? "mx-4" : "mx-24"
        }`}
      >
        <Link to="/carta" className="flex w-20 h-14">
          <div className="backButton flex justify-center items-center w-20 h-14 border-2 border-solid border-[#ef8114] rounded-full cursor-pointer bg-white">
            <svg
        ...
        <h1
          className={`h1Font ${
            isXs || isSm ? "text-[7vw]" : "text-5xl"
          } text-center text-[#502314]`}
        >
