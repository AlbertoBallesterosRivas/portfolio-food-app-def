const DeliveryPrice = ({ total, delivery, setDelivery, totalGlobal, setTotalGlobal }) => {
  ...
  let deliveryPriceMessage = null;
  if (total < 10) {
    setDelivery(4.99)
    setTotalGlobal(total + 4.99)
    // deliveryPrice = 4.99;
    deliveryPriceMessage = priceMessage(10, delivery);
  } else if (total < 15) {
    setDelivery(2.99)
    setTotalGlobal(total + 2.99)
    //deliveryPrice = 2.99;
    deliveryPriceMessage = priceMessage(15, delivery);
  } else if (total < 20) {
    setDelivery(1.99)
    setTotalGlobal(total + 1.99)
    //deliveryPrice = 1.99;
    deliveryPriceMessage = priceMessage(20, delivery);
  } else {
    setTotalGlobal(total)
  }
  console.log("TOTAL", total, "DELIVERYPRICE", delivery);
  return (
